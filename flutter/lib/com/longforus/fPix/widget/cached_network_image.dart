...
class CachedNetworkImageProvider
  ...
  CachedNetworkImageProvider(this.url,
      {this.cacheKey, this.scale: 1.0, this.errorListener, this.headers}) {
    assert(url != null);
    assert(scale != null);
    if (cacheKey == null || cacheKey!.isEmpty) {
      cacheKey = url;
  ...
  Future<ui.Codec> _loadAsync(CachedNetworkImageProvider key) async {
    ...
    var file =
        await cacheManager.getFile(url, cacheKey: cacheKey, headers: headers);
    if (file == null) {
  ...
  bool operator ==(dynamic other) {
    ...
    return cacheKey == typedOther.cacheKey && scale == typedOther.scale;
  }

  @override
  int get hashCode => hashValues(cacheKey, scale);
